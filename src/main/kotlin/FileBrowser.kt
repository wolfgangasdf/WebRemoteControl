...
object FileBrowser {
    ...
    private val filter = FilenameFilter { file: File, name: String ->
        ...
        if (name.startsWith("."))
            res = false
        else if (!file.isDirectory && name.contains(".")) if (!openExtensions.contains(name.substring(name.lastIndexOf(".") + 1)))
            res = false
        res
    }
